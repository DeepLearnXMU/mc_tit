...
class TransformerConfig(FairseqDataclass):
    ...
    # transalation pretrain
    transalation_checkpoint: str = field(
        default=None,
        metadata={"help": "The file of the translation model checkpoint"},
    )
    
    # multimodal
    img_feat_dim: Optional[int] = field(
        default=768, metadata={"help": "dimensions of image features"}
    )

    codebook_size: Optional[int] = field(
        default=500, metadata={"help": "size of the codebook"}
    )
    
    # stage 2
    mask_probability: float = field(
        default=0.15, metadata={"help": "The mask ratio of encoder out"}
    )
    # stage 3
    add_linear: bool = field(
        default=False,
        metadata={"help": "add an linear layer to determine whether the text and image representations are consistent"},
    )
    discriminator_hidden_dim: int = field(
        default=2048, metadata={"help": "hidden dimension of the discriminator"}
    )
    discriminator_max_step: int = field(
        default=100000, metadata={"help": "max warmup step of the discriminator"}
    )
