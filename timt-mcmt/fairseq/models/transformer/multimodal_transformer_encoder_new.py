...
class Quantize(nn.Module):
    ...
    def forward(self, input, ret_indices = True, freeze = False):
        ...
        import numpy as np
        np.random.seed(21)
        def generate_random_indices(embed_ind, total_indices=2048):
            batch_size, embed_size = embed_ind.size()

            # 创建所有可能的索引
            all_indices = torch.arange(total_indices)

            # 获取embed_ind中出现的索引
            unique_indices = torch.unique(embed_ind)

            # 计算未出现在embed_ind中的索引
            missing_indices = torch.tensor(list(set(all_indices.cpu().numpy()) - set(unique_indices.cpu().numpy())))

            # 随机选择与batch_size相同数量的索引
            rand_indices = missing_indices[torch.randint(0, len(missing_indices), (batch_size, embed_size))].to(embed_ind.device)

            return rand_indices.view(batch_size, embed_size)
